...
public class MainReportService implements ReportBusinessService {
    ...
    public <T extends LogicalDocument> void persist(T report) {
        ...
        final String maxS = environment.getProperty("maxNumberOfReports");
        int max;
        try {
        ...
        if (max > -1 && findAll().size() >= max) {
            throw new MaxNumberOfReportsReachedException("The limit of " + max + " reports has been reached.");
        } else {
