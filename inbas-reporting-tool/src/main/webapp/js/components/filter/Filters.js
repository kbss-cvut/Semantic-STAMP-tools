...
function _resolveUniqueFilterableValues(data) {
    ...
    for (let i = 0, len = data.length; i < len; i++) {
        ...
        ((item) => {
            _processFilters((filter) => {
                const value = Utils.getPropertyValue(item, filter.path);
    ...
    _renderFilters() {
        ...
        _processFilters((filter) => {
            if (filter.type === 'select') {
                // Do not render the filter if there are no options for it or if there is only one option, because then it
                // will have no effect
                // One of the options is the default one
                if (this.state[filter.options].length < 3) {
                    return;
                }
